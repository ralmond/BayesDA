...
Let $n = \alpha+\beta$ and $p = \alpha/n$.  This is an alternative parameterization in terms of mean, and _effective sample size_.
...
Now consider the dependent collection of random variables
$\theta_1, \ldots, \theta_K$, where $\theta_k = \alpha_k/(\sum_k \alpha_k)$.
...
$$ f(\theta_1,\ldots,\theta_K) = \frac{\Gamma(\alpha_1 + \cdots + \alpha_K)}{\Gamma(\alpha_1)\cdots\Gamma(\alpha_K)} \theta_1^{\alpha_1 -1}\cdots\theta_K^{\alpha_K-1}$$
Note that each component follows a beta distribution with parmeters $\alpha=\alpha_k$ and $\beta=\sum_{k'\not=k} \alpha_{k'}$.

Once again, the sum of the alphas is an _effective sample size_ and this distribution can be parameterized via its mean vector and effective sample size.
...
alphas <- c(3,2,1)
ddirich <- function (a,b,c) MCMCpack::ddirichlet(c(a,b,c),alphas)
