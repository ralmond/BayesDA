...
data = list(N = N, J = J, newpid = newpid, CD4PCT_ik = CD4PCT, VDATE_1ik = VDATE)


fileName <- "Bayesian/GH16.3.stan_code.stan"
#stan_code <- readChar(fileName, file.info(fileName)$size)
#cat(stan_code)


resStan <- stan(fileName, data = data, chains = 3, iter = 3000, warmup = 500, thin = 10)

library(shinystan)
launch_shinystan(resStan)

plot(resStan,pars=c("beta_0","beta_1","sigma_e0","sigma_u0k"))
pairs(resStan,pars=c("sigma_u0k","u_0k[1]","u_0k[2]"))
pairs(resStan,pars=c("beta_0","sigma_u0k","beta_0k[1]","beta_0k[2]"))
